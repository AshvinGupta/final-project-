...
function setup() {
  ...
  function draw() {
    background(50,50,50);
  if (keyDown("e")){
bulletshot = createSprite(400,400,10,20);
bulletshot.addAnimation("bullet",bullets);
  }
  plane.velocityY = World.mouseY
...
function spawnclouds(){
    if(frameCount %100===0){
        cloud = createSprite(800,750,50,10);
...
//}
//function gameOver(){
  //if (enemy1.isTouching(plane)||enemy2.isTouching(plane)||enemy3.isTouching(plane)){
    //gameState = end
   // gameOver = createSprite(width/2,height/2-40,20,20);
   // gameOver.addImage(goImg);
   // gameOver.scale = 0.1;
